...
int main(void) {
	...
	yyjson_doc* doc  = yyjson_read_file("config.json", flg, NULL, &err);
	...
	yyjson_val* p_exchange = yyjson_obj_get(root, "exchanges");

	array_size = yyjson_arr_size(p_exchange);
	...
	p_threads = malloc(sizeof(pthread_t) * array_size);
